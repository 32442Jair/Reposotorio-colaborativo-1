...
public: 
    ...
    double parse(){ double v=expr(); skip(); if(p!=s.size()) throw runtime_error("Entrada 
invalida."); return v; } 
}; 
...
int decidirEnteroDesdeDouble(double val, const string& campo){ 
    ...
    for(;;){ 
        ...
        if(op=="3"){ double v=leerNumeroExpr("Nuevo valor: "); return 
decidirEnteroDesdeDouble(v,campo); } 
...
fprintf(f,"ID,Nombre,Prioridad,MemoriaKB,Variables,Condiciones,Datos,Pide,R6.1,R6.2,R6.3,Evi
 dencia\n"); 
        Proceso* a = cabeza; 
        while(a){ 
            string v=a->variables, c=a->condiciones, d=a->datos, p=a->pide, ev=a->evidencia, 
n=a->nombre; 
            size_t k; 
...
public: 
    ...
    void encolar(Proceso* p) { 
        ...
        p->evidencia += "encolar: metodo=cola_prioridad(descendente); justificacion=atender 
mayor prioridad primero\n"; 
    ...
    Proceso* desencolar() { 
        ...
        p->evidencia += "ejecutar: metodo=cola_prioridad; justificacion=orden por prioridad; 
resultado=ok\n"; 
...
int main(){ 
    ...
    for(;;){ 
        ...
        if (op == 1) { 
            ...
            int idNuevo = lista.insertarProceso(nombre, pr, mem, variables, condiciones, datos, 
pide); 
 
            Proceso* p = lista.buscarPorId(idNuevo); 
            if(p){ 
                p->evidencia += "creacion: metodo=lista_enlazada; justificacion=insertar al final; 
resultado=ok\n"; 
            } 
        } 
        else if (op == 2) { 
            ...
            if (p){ 
                ostringstream oss; 
                oss << "memoria: metodo=pila(LIFO); justificacion=liberar en orden inverso; size=" << 
tam << "KB\n"; 
